...
import './Styles/StyleTableFeedback.css'
import imagen10 from './img/imagen1.jpg'
...
export default function TableFeedback() {
  ...
  const añadirComentario = () => {
    if (newComment.trim() !== "") {
      setUsuarioss(prevUsuarios => [ //Son ejemplos de como queda, no es necesario esto Debido a que no se ha realizado el consumo de la Api
        ...prevUsuarios,
            ...
            {Usuarios.map((usuario, index) => (
              <tr key={index} className='tr'>
                <td className='user-cell td'>
                  {usuario.imagen ? (
                    <img src={usuario.imagen} alt={usuario.name} className='perfiles-imagen' />
                  ) : (
